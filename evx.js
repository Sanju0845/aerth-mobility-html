(function () {
  ...
  /* =====================================================
     EVXPERTZ – Premium Interactions & Scroll Reveal
     ===================================================== */
  ...
  (function () {
    ...
    function initReviewsCarousel(raw) {
      ...
      // Extract short area name from location string, e.g. "EVXpertz (KPHB) - …" → "KPHB"
      function shortLocation(loc) {
        if (!loc) return '';
        var m = loc.match(/\(([^)]+)\)/);
        var retLoc = m ? m[1] : loc.split(' - ')[0].replace(/EVXpertz\s*/i, '').trim();
      ...
      fetch('locations/locations.json')
        ...
        .then(function (locations) {
          ...
          locations.forEach(function (loc) {
            ...
            var isHQ = loc.code === 'EVXPERTZ-001';
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      (async function () {
        try {
          ...
          console.log('EVXPERTZ Enquiry:', formData);
